	...
	cv::Mat WhitePixelExtraction(cv::Mat src) {
		...
		for (size_t j = 0; j < src.rows - 0; j++) {
			for (size_t i = 0; i < src.cols - 0; i++) {
				...
				if (intensity >= thetal && intensity - src.at<uchar>(j, i - dist) > thetad &&
					intensity - src.at<uchar>(j, i + dist) > thetad) {
				...
				else if (intensity >= thetal && intensity - src.at<uchar>(j - dist, i) > thetad &&
					intensity - src.at<uchar>(j + dist, i) > thetad) {
					dst.at<uchar>(j, i) = 255;
				}
				else {
					dst.at<uchar>(j, i) = 0;
